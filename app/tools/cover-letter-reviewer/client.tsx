...
const LENGTH_STYLES: Record<string, string> = {
  appropriate: "bg-green-500/10 border-green-500/20 text-green-400",
  "too short": "bg-yellow-500/10 border-yellow-500/20 text-yellow-400",
  "too long": "bg-yellow-500/10 border-yellow-500/20 text-yellow-400",
};
...
export function CoverLetterClient() {
  ...
  const analyze = async () => {
    ...
    try {
      ...
      if (file) {
        ...
        formData.append("tool", "cover-letter-reviewer");
  ...
  const buttonLabel = () => {
    ...
    if (file) return jd.trim() ? "Scan File Against This JD" : "Scan File";
    return jd.trim() ? "Review Against This JD" : "Review My Cover Letter";
  };
  ...
  return (
    <div className="max-w-[720px] mx-auto px-6 py-10">
      ...
      <div className="flex items-center gap-3.5 mb-2">
        ...
        <h1 className="font-heading text-2xl md:text-3xl font-bold text-[var(--text)]">Cover Letter Reviewer</h1>
      </div>
      <p className="text-muted text-sm leading-relaxed mb-7">
        Paste or upload your cover letter below. Add a job description for targeted ATS keyword
        matching and role-fit analysis against that specific role.
      </p>
      ...
      <textarea
        ...
        placeholder={
          file
            ? "Optional: add context or the role you&apos;re applying to…"
      ...
      {!file && input.length > CHAR_LIMIT && (
        <div className="mb-3 p-3 rounded-lg bg-yellow-500/10 border border-yellow-500/30 text-yellow-300 text-sm">
          ⚠️ Your cover letter is very long — only the first {CHAR_LIMIT.toLocaleString()} characters will be reviewed.
        </div>
      )}
        ...
        {!file ? (
          <div>
            ...
            <p className="mt-1.5 text-muted text-xs text-center">
              💡 Upload your cover letter as PDF or DOCX, or paste the text above.
            </p>
          </div>
        ) : (
      ...
      <div className="mt-5">
        <label className="block font-heading text-sm font-semibold text-[var(--text)] mb-1">
          Paste a job description <span className="text-muted font-normal">(optional)</span> — get targeted role-match feedback
        </label>
        <p className="text-muted text-xs mb-2">
          Leave blank for general feedback. Paste a JD to unlock ATS keyword matching and company/role alignment analysis.
        </p>
        <textarea
          value={jd}
      ...
      {result && (
        <div className="mt-7 bg-surface border border-border rounded-2xl p-7 animate-fade-up">
          ...
          <div className="flex items-center gap-6 mb-6">
            ...
            <div>
              ...
              {result.lengthAssessment && (
                <span className={`inline-block mt-1.5 px-2 py-0.5 rounded-full border text-xs font-mono ${LENGTH_STYLES[result.lengthAssessment] || "bg-border text-muted border-border"}`}>
                  Length: {result.lengthAssessment}
                </span>
              )}
            </div>
          </div>
          ...
          {/* Role match / keyword gap */}
          {result.matchAgainstRole && (
            <div className="mt-5 pt-5 border-t border-border">
              <h4 className="font-heading text-sm font-semibold text-[var(--text)] mb-1">
                {result.matchAgainstRole.inferredRole
                  ? `Keyword Gaps for ${result.matchAgainstRole.inferredRole} Roles`
                  : "Role & Company Match"}
              </h4>

              {result.matchAgainstRole.inferredRole && (
                <p className="text-muted text-xs mb-3 italic">
                  💡 We inferred you&apos;re targeting <strong className="text-[var(--text)]">{result.matchAgainstRole.inferredRole}</strong> roles.
                  Paste a specific JD above for more targeted keyword and role-fit analysis.
                </p>
              )}

              {result.matchAgainstRole.companyAlignment && (
                <div className="mb-3 p-3 rounded-lg bg-surface border border-border">
                  <p className="text-xs font-mono text-muted mb-1">Company alignment:</p>
                  <p className="text-[var(--text)] text-sm leading-relaxed">{result.matchAgainstRole.companyAlignment}</p>
                </div>
              )}

              {result.matchAgainstRole.roleAlignment && (
                <div className="mb-3 p-3 rounded-lg bg-surface border border-border">
                  <p className="text-xs font-mono text-muted mb-1">Role alignment:</p>
                  <p className="text-[var(--text)] text-sm leading-relaxed">{result.matchAgainstRole.roleAlignment}</p>
                </div>
              )}

              {(result.matchAgainstRole.missingKeywords?.length ?? 0) > 0 && (
                <div className="mb-3">
                  <p className="text-xs font-mono text-muted mb-1.5">Missing keywords:</p>
                  <div className="flex flex-wrap gap-1.5">
                    {result.matchAgainstRole.missingKeywords!.map((kw, i) => (
              ...
              {(result.matchAgainstRole.strongOverlaps?.length ?? 0) > 0 && (
                <div className="mb-3">
                  <p className="text-xs font-mono text-muted mb-1.5">Strong matches:</p>
                  <div className="flex flex-wrap gap-1.5">
                    {result.matchAgainstRole.strongOverlaps!.map((kw, i) => (
              ...
              {(result.matchAgainstRole.rewriteHints?.length ?? 0) > 0 && (
                <div>
                  <p className="text-xs font-mono text-muted mb-1.5">Diagnostic observations (rewrites are paid tier):</p>
                  {result.matchAgainstRole.rewriteHints!.map((hint, i) => (
      ...
      <section className="mt-14 border-t border-border pt-10">
        <h2 className="font-display text-2xl text-[var(--text)] mb-3">How the AI Cover Letter Reviewer works</h2>
        <p className="text-muted text-sm leading-relaxed mb-4">
          Paste your cover letter or upload it as a PDF or DOCX file. Our AI evaluates your letter
          across four dimensions: the strength of your opening hook, the evidence and structure of
          your body paragraphs, the clarity of your closing call-to-action, and your overall tone
          and voice. It also scores your ATS compatibility — how well the letter will survive
          automated keyword screening — and flags length issues (the sweet spot is 250–400 words).
          Paste a job description to unlock company and role alignment analysis: see exactly which
          keywords the role demands that your letter is missing, and get diagnostic observations
          on specific sentences to sharpen on your own.
        </p>
        <p className="text-muted text-xs leading-relaxed italic">
          Disclaimer: This tool provides general cover letter feedback and is not a substitute for
          professional career counseling. Results may vary based on industry, role, and hiring manager
          preferences.
        </p>
      </section>
    </div>
